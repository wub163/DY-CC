...
void usbapp_mode_stop(void)
{
	...
	{
		case USBH_MSC_MODE: 
//			free(MSC_Machine.buff);	//释放内存
//			free(USBH_DataInBuffer);	//释放内存
//			free(USBH_DataOutBuffer);	//释放内存
		case USBH_HID_MODE: 
...
//	usbapp_mode_stop();//先停止当前USB工作模式
	...
	{
		case USBH_MSC_MODE: 
//			MSC_Machine.buff=malloc(USBH_MSC_MPS_SIZE);
			USBH_DataInBuffer=malloc(512);
			USBH_DataOutBuffer=malloc(512);
		
//			USBH_CfgDesc=malloc(512); 
...
//			USBH_CfgDesc=malloc(512);
			...
			break;		
		case USBD_MSC_MODE:
//			MSC_BOT_Data=malloc(MSC_MEDIA_PACKET);
