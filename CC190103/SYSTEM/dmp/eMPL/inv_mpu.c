...
int mpu_init(void)
{
  ...
  //  }
   ...
   // Sensor_writeToReg(0xD0,st.reg->accel_cfg2,1,&data[0]);
 ...
 *  @return     0 if successful.
 */
int mpu_lp_accel_mode(u16 rate)
{
    unsigned char tmp[2];
 ...
 *  @return     0 if successful.
 */
int mpu_get_temperature(float *data, unsigned long *timestamp)
{
    unsigned char tmp[2];
    short raw;
...
//        get_ms(timestamp);
    ...
    *data =(float)raw / st.hw->temp_sens+st.hw->temp_offset ;
...
int mpu_set_gyro_fsr(unsigned short fsr)
{
    ...
    if (st.chip_cfg.gyro_fsr == (data >> 3))
        return 0;
  //  Sensor_writeToReg(0xD0,st.reg->gyro_cfg,&data);
    ...
    unsigned char *more)
{
    ...
    //i2c_read(st.hw->addr, 0x76,1,&tmp[1]);
...
int mpu_set_bypass(unsigned char bypass_on)
{
    ...
    if (bypass_on) {
       ...
       if (i2c_read(0xD0,st.reg->user_ctrl,1,&tmp))
            return -1;
        if (st.chip_cfg.sensors & INV_XYZ_COMPASS)
            tmp |= BIT_AUX_IF_EN;
        else
...
AKM_restore:
    ...
    i2c_write(st.chip_cfg.compass_addr, AKM_REG_ASTC,tmp);
    tmp[0] = SUPPORTS_AK89xx_HIGH_SENS;
    i2c_write(st.chip_cfg.compass_addr, AKM_REG_CNTL,tmp);
    mpu_set_bypass(0);
    return result;
}
#endif
...
#define LOAD_CHUNK  (1)
    ...
    for (ii = 0; ii < length; ii += this_write) {
      ...
      this_write=1;
    top:      if(mpu_write_mem(ii, this_write, (unsigned char*)&firmware[ii]))
           return -1;
    ...
    unsigned long lpa_freq)
{
    ...
    if (lpa_freq) {
        ...
        if (i2c_write(st.hw->addr, st.reg->motion_thr,1,data))
            goto lp_int_restore;
        if (i2c_write(st.hw->addr, st.reg->motion_dur,1,&data[1]))
            goto lp_int_restore;
        ...
        if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1,1,data)&&i2c_write(st.hw->addr, st.reg->pwr_mgmt_2,1,&data[1]))
            goto lp_int_restore;
