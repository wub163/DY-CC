...
{
 	u32 r=0;
	if(x>=LCD_PIXEL_WIDTH||y>=LCD_PIXEL_HEIGHT)return 0;	//超过了范围,直接返回		   
	  uint32_t  Xaddress =0;
    Xaddress =  CurrentFrameBuffer + 3*(LCD_PIXEL_WIDTH*y + x);
	 r= *(__IO uint16_t*) (Xaddress+2);        //GB
	 r= (r<<16)|((*(__IO uint16_t*) Xaddress)&0x00ffff);        //GB
	return r;					
					//其他IC
}
...
{
	if(x>=LCD_PIXEL_WIDTH||y>=LCD_PIXEL_HEIGHT)return ;	//超过了范围,直接返回		   
	  uint32_t  Xaddress =0;
    Xaddress =  CurrentFrameBuffer + 3*(LCD_PIXEL_WIDTH*y + x);
	  *(__IO uint16_t*) Xaddress= (0x00FFFF & color);        //GB
  	*(__IO uint8_t*)( Xaddress+2)= (0xFF0000 & color) >> 16; //R
}
...
//LCD_SetCursor(500,500);
//  LCD_DisplayStringLine(LINE(1),100,(uint8_t* )"BH 5.0 inch LCD para:");
...
////	
////	LCD_SetColors(LCD_COLOR_BLUE,LCD_COLOR_BLACK);
////  LCD_DrawFullRect(200,350,50,130);
////  
////  Delay(0xFFFFFF);
////  
////	LCD_SetColors(LCD_COLOR_BLACK,LCD_COLOR_BLACK);
