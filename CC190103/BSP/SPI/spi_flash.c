...
void sFLASH_Init(void)
{
  ...
  SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_2;       //频率84m/2=42m
...
void sFLASH_WriteBuffer(uint8_t* pBuffer, uint32_t WriteAddr, uint16_t NumByteToWrite)
{
  ...
  if (Addr == 0) /*!< WriteAddr is sFLASH_PAGESIZE aligned  *///起始地址页对齐。
  {
    if (NumOfPage == 0) /*!< NumByteToWrite < sFLASH_PAGESIZE *///单页写
    {
      sFLASH_WritePage(pBuffer, WriteAddr, NumByteToWrite);
    }
    else /*!< NumByteToWrite > sFLASH_PAGESIZE */               //多页写
    {
      while (NumOfPage--)
      {
  ...
  {
    if (NumOfPage == 0) /*!< NumByteToWrite < sFLASH_PAGESIZE *///单页
    {
      if (NumOfSingle > count) /*!< (NumByteToWrite + WriteAddr) > sFLASH_PAGESIZE *///跨页
      {
    ...
    else /*!< NumByteToWrite > sFLASH_PAGESIZE */               ///多页
    {
